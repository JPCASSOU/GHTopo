...
type
  ...
  private
    ...
    function GenererListeDesSystemes(const DoDispSysts: boolean): boolean;
    function GetIndexSysteme(const Mode: byte; const qCodeEPSG: integer): integer;
    // conversions
    ...
    function GetIdxEllipsoideByCodeEPSG(const C: integer): integer;
    procedure AddEllipsoideByParameters(const qCodeEPSG: integer;
    ...
    function GetIdxDatumByCodeEPSG(const C: integer): integer;

    procedure AddDatumByParameters(const qCodeEPSG: integer;
    ...
    function GetIdxProjectionByCodeEPSG(const C: integer): integer;
    procedure AddProjectionByParameters(const qCodeEPSG: integer; const qNatureProj, qNom, qDescription, qDatum: string; const qParametres: TGHStringArray);
    // multiples
  ...
  public
    ...
    procedure ReconstruireProjectionsDepuisDonneesEnDur(const DoDispMsgCtrl: boolean);
    //**************************************************************************
...
{ TConvertisseurJPC }
function TConversionSysteme.GetIndexSysteme(const Mode: byte; const qCodeEPSG: integer): integer;
begin
  Result := -1;
  case Mode of
...
begin
  ...
  try
    ...
    ReconstruireProjectionsDepuisDonneesEnDur(miaou);  // reconstruction en dur
    // ++LoadFromFichier(FichierProjections); // charger depuis fichier
    GenererListeDesSystemes(miaou);           // générer la liste des systèmes de coordonnées
    if (miaou) then Tester();                     // test avec un couple de données
    FIsReady := true;
    Result := True;
  except
...
function TConversionSysteme.GetIdxEllipsoideByCodeEPSG(const C: integer): integer;
var
  i: integer;
  EWE: TEllipsoide;
begin
...
procedure TConversionSysteme.AddEllipsoideByParameters(const qCodeEPSG: integer;
...
function TConversionSysteme.GetIdxDatumByCodeEPSG(const C: integer): integer;
var
  i: Integer;
  EWE: TDatum;
begin
...
function TConversionSysteme.GetIdxProjectionByCodeEPSG(const C: integer): integer;
var
  i: Integer;
  EWE: TProjection;
begin
...
procedure TConversionSysteme.AddProjectionByParameters(const qCodeEPSG: integer;
...
begin
  try
    ...
    if (WU >= 0) then //Raise ERangeError.CreateFmt('Datum %s inconnu dans la projection %s',  [qDatum, qNom]);
    begin
      MyProj.Datum := GetDatum(WU);
      // décodage des paramètres
      Compteur := 0;
      for i := 0 to MyProj.Nb_ParamReal - 1    do MyProj.ParamReal[i]    := ConvertirEnNombreReel(qParametres[i], 0.00);
...
begin
  try
    ...
    // décodage des paramètres
    Compteur := 0;
    for i := 0 to MyMultiple.Nb_ParamReal - 1    do MyMultiple.ParamReal[i]    := ConvertirEnNombreReel(qParametres[i], 0.00);
...
//******************************************************************************
function TConversionSysteme.GenererListeDesSystemes(const DoDispSysts: boolean): boolean;
var
  LS: TStringList;
  i: Integer;
  EWE: TGHStringArray;
  ...
  PPP: TProjection;
  MMM: TMultiple;
  procedure S666(const S: string);
  begin
    if (DoDispSysts) then AfficherMessage(S);
  end;
...
begin
  ...
  try
    ...
    S666(Format('-- Datums: %d', [Nb]));
    for i:= 0 to Nb - 1 do
    begin
      DDD := GetDatum(i);
      S666(Format('%d: %d: %s', [i, DDD.CodeEPSG, DDD.Nom]));
      LS.Add(Format('%d: %s', [DDD.CodeEPSG, DDD.Nom]));
    end;
    S666('');
    Nb := GetNbProjections;
    S666(Format('-- Projs: %d', [Nb]));
    for i:= 0 to Nb - 1 do
    begin
      PPP := GetProjection(i);
      S666(Format('%d: %d: %s', [i, PPP.CodeEPSG, PPP.Nom]));
      LS.Add(Format('%d: %s', [PPP.CodeEPSG, PPP.Nom]));

    end;
    S666('');
    Nb := GetNbMultiples;
    S666(Format('-- Multiples: %d', [Nb]));
    for i:= 0 to Nb - 1   do
    begin
      MMM := GetMultiple(i);
      S666(Format('%d: %d: %s', [i, MMM.CodeEPSG, MMM.Nom]));
      LS.Add(Format('%d: %s', [MMM.CodeEPSG, MMM.Nom]));

      S666(WU);
    end;
    S666(Format('-- Systemes: %d', [LS.Count]));
    ...
    begin
      ...
      FListeDesSystemes[i].NomEPSG  := Trim(EWE[1]);
    end;
    // vidage
    LS.Clear;
    //--------------
    Result := True;
    S666('---------------------------------------------------');
    S666(Format('%s.GenererListeDesSystemes OK', [ClassName]));
    S666('---------------------------------------------------');
    Exit;
  finally
    LS.Free;
...
procedure TConversionSysteme.ReconstruireProjectionsDepuisDonneesEnDur(const DoDispMsgCtrl: boolean);
CONST
  DUMMY = 0.00;
var
  EWE: TGHStringArray;
...
var
  ...
  begin
    AfficherMessage(Format(' -- %s: %.6f, %.6f', [S, P.U, P.V]));
  end;
  procedure miouXY(const S: string; const P: TProjUV; const QX, QY: double);
  var
     Delta: TProjUV;
  begin
    Delta.U := QX - P.U;  Delta.V := QY - P.V;
    AfficherMessage(Format(' -- %s: %.6f, %.6f (expected: %.2f, %.2f) (delta: %.2f, %.2f)', [S, P.U, P.V, QX, QY, Delta.U, Delta.V]));
  end;
begin
  ...
  AfficherMessage(Format('Fuseau UTM: %d', [GetNumeroFuseauUTM(EWE_WGS84.U, EWE_WGS84.V)]));
  AfficherMessage('Avec EPSG');
  AfficherMessage('*********');
...
begin
  ...
  try
    ...
    begin
      ...
      case IndexOfString(EWE[0], True, ['ellipsoide', 'datum', 'projection', 'multiple']) of
        0:
        begin
          // # ELLIPSOIDES
          //Nom  Description  a  e
          ...
          AddEllipsoideByParameters(-1,
            EWE[1],
            EWE[2],
            ConvertirEnNombreReel(EWE[3], -1.00),
            ConvertirEnNombreReel(EWE[4], -1.00));
        end;
        1:
        begin
          //# DATUMS
          ...
          AddDatumByParameters(-1, EWE[1], EWE[2], EWE[3],
            ...
            ConvertirEnNombreReel(EWE[9], 0.00),
            ConvertirEnNombreReel(EWE[10], 1.00),
            EWE[11]);
        end;
        2:
        begin
