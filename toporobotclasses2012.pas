...
//*)
    ...
    EWE := SplitFixedColLine(MyLine, [1, 11, 22, 33, 41, 50, 57, 100]);
    // Autre syntaxe du signal d'arrêt: un nombre négatif (mettre -999 dans le fichier)
    if (StrToIntDef(EWE[0], 0) < -2) then exit(errLnSTOP_SIGNAL);
...
begin
  ...
  WU := Format('%s.LoadFromPocketTopoTXT: %s', [ClassName, FichierTXT]);
  AfficherMessage(WU + ' - Fonctionnalite En attente');
