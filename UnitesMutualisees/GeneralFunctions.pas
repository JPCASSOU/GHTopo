...
// obtenir le dossier de GHCaveDraw
...
procedure AfficherMessage(const Msg: string); overload;
procedure AfficherMessage(const ClassName, Msg: string); overload;
...
function InterpreterTexteAnnotation(const STR: string;
                                    const NbC: integer;
                                    const Station: TBaseStation): string;
...
// retourne les numéros de serie:station d'un point topo
function GetToporobotIDStation(const St: TBaseStation): TToporobotIDStation;
function GetToporobotIDStationAsString(const St: TBaseStation; const WithIDTerrain: boolean): string;
...
// conversion d'attributs de fonte
function AtrributsFontePas2GHCD(const FA: TFontStyles): integer;
function AtrributsFonteGHCD2Pas2(const FA: Integer): TFontStyles;
// pour SVG
...
// échappement des caractères spéciaux de MySQL
// même utilité que la fonction mysqli_real_escape_string() du PHP
// -> on en reprend le même nom.
function mysqli_real_escape_string(const S: string): string;
// échappement des attributs texte pour XML
...
// retourne un angle en degrés dans le bon quadrant et sur [0; 360[
...
procedure AfficherMessage(const ClassName, Msg: string);
begin
  AfficherMessage(Format('%s: %s.%s',[TimeToStr(Now), Classname, Msg]));
end;
procedure AfficherMessageErreur(const Msg: string);
begin
  {$IFDEF MSWINDOWS}
    ...
    begin
      Lines.Add(TimeToStr(Now) + ': ' + Msg);
      if (Lines.Count > 500) then Lines.Delete(0);
      Refresh;
    end;
    {$ENDIF} // {$IFDEF MSWINDOWS}
  {$ENDIF}
...
begin
  ...
  {$ENDIF}
  {$IFDEF MAC_OS_X_VERSION_MIN_REQUIRED}
  // Sous MacOSX, coder en dur le chemin de GHCaveDraw
  Result := '~/Desktop/0_LogicielsJPC/GHCaveDraw/';
  {$ENDIF}
  {$IFDEF LINUX}
  Result := ExtractFilePath(ParamStr(0));
  {$ENDIF}
...
function IsInBoundingBox(const XX, YY: Double; const BB: TBoundingBox): boolean;
begin
  Result := (IsInRange(XX, BB.C1.X, BB.C2.X) AND
             IsInRange(YY, BB.C1.Y, BB.C2.Y));
...
function LLANFAIR_PG(const STR: string;
                     const NbC: integer;
                     const Alt: double;
                     const StationName: string): string;
//*)
function InterpreterTexteAnnotation(const STR: string;
                                    const NbC: integer;
                                    const Station: TBaseStation): string;
...
begin
  ...
  // %L => affichage de la longueur
  // %A => affichage de l'azimut
  // %P => affichage de la pente
  // %I => Affichage de l'ID interne de la station

  // ID station
  S666 := zdar(S666, '%s', qTRB.aIDTerrain); ////Station.IDTerrain);
  S666 := zdar(S666, '%i', Format('[%d.%d]', [qTRB.aSerie, qTRB.aStation]));
  // coordonnées
  S666 := zdar(S666, '%x', Format('X = %.0n m',[Station.PosStation.X]));
  S666 := zdar(S666, '%y', Format('Y = %.0n m',[Station.PosStation.Y]));
  S666 := zdar(S666, '%z', Format('Alt %.0n m',[Station.PosStation.Z]));
  // longueur, az, pente
  S666 := zdar(S666, '%l', Format('L=%.1fm',[Dist]));
  S666 := zdar(S666, '%a', Format('A=%.1fd',[Az]));
  S666 := zdar(S666, '%p', Format('P=%.1fd',[Inc]));

  //---------------
  try
    if (Length(S666)=0) or (NbC=-1) then  begin Result:=S666; exit; end;
...
begin
  ...
  begin
    S1 := St.IDStation div DIV_SR_ANTENNE;
    S2 := St.IDStation mod DIV_SR_ANTENNE;
    Result.aSerie   := S1;
...
begin
  EWE := GetToporobotIDStation(ST);
  //AfficherMessage(Format('"%s"', [EWE.aIDTerrain]));
...
function GenererLigneSimpleLigne(const QIdx: integer; const L: TSimpleLigne): string;
begin
  Result := Format('    line '+ #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER + //ID, Groupe, Style
                                #9 + FORMAT_NB_INTEGER + #9 + '%.2f' + #9 + '%.2f' + #9 + '%.2f' +
                                #9 + FORMAT_NB_INTEGER + #9 + '%.2f' + #9 + '%.2f' + #9 + '%.2f' +
                                #9 + '%s',
...
function GenererLigneSymbole(const QIdx: integer; const Obj: TSymbole): string;
begin
  Result := Format('    ponctualobject '+ #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER +  #9 + FORMAT_NB_INTEGER + //IDXGroupe, TypeObject, Couleur,
                                #9 + FORMAT_NB_INTEGER + #9 + '%.2f' + #9 + '%.2f' + #9 + '%.2f' + // IDBase, Offset
                                #9 + '%.2f' + #9 + '%.2f' + #9 + '%.2f' +
                                #9 +'%s' + // texte
                                #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER + #9 + '%s',
...
function GenererLigneTexte(const QIdx: integer; const Obj: TTextObject): string;
begin
  Result := Format('    text '+ #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER + //ID, Groupe, Style
                                #9 + FORMAT_NB_INTEGER + #9 + '%.2f' + #9 + '%.2f' + #9 + '%.2f' +
                                #9 + FORMAT_NB_INTEGER + #9 + FORMAT_NB_INTEGER+ #9 + '%s' + #9 + '%s',
...
function GenererLigneGroupe(const QIdx: integer; const Grp: TGroupeEntites): string;
begin
  Result := Format('  groupe'+ #9 +FORMAT_NB_INTEGER + #9 +
                       '%s' + #9 + FORMAT_NB_INTEGER + #9 +
                       FMT_COORDS + #9 +
                       FORMAT_NB_INTEGER + #9 + '%s' +
                       #9 + FORMAT_NB_INTEGER +
                       #9 + '%.2f' +
                       #9 + '%s', [
                         GRP.IDGroupeEntites,
...
begin
  Result := Format('  ' + SCRAP + FMT_SCRAP, [QIdx, S.IDGroupe,
                                                   GetRValue(S.Couleur), GetGValue(S.Couleur), GetBValue(S.Couleur),
                                                   S.Opacite,
                                                   S.Nom]);
end;
