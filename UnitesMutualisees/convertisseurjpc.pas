...
type
  ...
  public
    ...
    function ConversionSyst1ToSyst2EPSG(const SystSrc, SystCible: TCodeEPSG; const MyPoint: TProjUV): TProjUV;


    function GetEPSGSystemeFromCodeEPSG(const C: TCodeEPSG): TLabelSystemesCoordsEPSG;
    function GetIndexSystemeByCodeEPSG(const C: TCodeEPSG): integer;
...
begin
  ...
  try
    ...
    AfficherMessageErreur(Format('Erreur dans %s.Initialiser', [ClassName]));
  end;
  FIsReady := Result;
end;
...
begin
  ...
  begin
    EWE := GetEllipsoide(i);
    begin
      if (LowerCase(EWE.Nom) = LowerCase(qNom)) then Exit(i);
    end;
  end;
end;
...
begin
  ...
  begin
    EWE := GetEllipsoide(i);
    begin
      if (TCodeEPSG(EWE.CodeEPSG) = C) then Exit(i);
    end;
  end;
end;
...
begin
  try
    ...
    for I := 0 to MyMultiple.Nb_ParamInteger - 1 do MyMultiple.ParamInteger[I] := StrToIntDef(qParametres[I + Compteur], 0);
...
function TConversionSysteme.ConversionSyst1ToSyst2EPSG(const SystSrc, SystCible: TCodeEPSG; const MyPoint: TProjUV): TProjUV;
var
...
begin
  ...
  if (EWE.U < 1000.00)  then EWE.U := EWE.U * 1000.00; // convertir éventuellement en mètres
  if (EWE.V < 10000.00) then EWE.V := EWE.V * 1000.00; // convertir éventuellement en mètres
...
function TConversionSysteme.GetEPSGSystemeFromCodeEPSG(const C: TCodeEPSG): TLabelSystemesCoordsEPSG;
var
  EWE: TLabelSystemesCoordsEPSG;
  i: Integer;
begin
...
function TConversionSysteme.GetIndexSystemeByCodeEPSG(const C: TCodeEPSG): integer;
var
  i, Nb: Integer;
  EWE: TLabelSystemesCoordsEPSG;
begin
