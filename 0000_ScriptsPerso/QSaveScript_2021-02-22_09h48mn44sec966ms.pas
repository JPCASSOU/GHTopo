...
begin
  ...
  QReturnCode := -1;
  QReturnMsg  := '';
  QPath       := '/www/0000_TotoDummy/';   // dans le système de fichiers distant
  QFilename   := '';
  if (not InputPasswordQuery('Connexion au serveur FTP', 'Mot de passe', QPassWord)) then exit;
  //'function  InputQuery(const ACaption, APrompt: string; var QValeur: string): boolean;');
  if (FTP_BeginConnexion('ftp4.phpnet.org', 21, 'synclinal65', QPassWord, QReturnCode, QReturnMsg)) then
  begin
