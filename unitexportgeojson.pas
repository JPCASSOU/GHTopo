...
{ TGeoJSONExport }
 ...
 public
   ...
   procedure AddLayer(const L: string);
...
begin
 ...
 // WriteLine(Format('      "%s": "%s",', ['name', Name]));
  ...
  case P of
    tpPOLYLINE: ; // Pas de multipolylignes
    tpPOLYGON : WriteLine(       '        ['); // support du multi-polygones
  end;
end;
...
procedure TGeoJSONExport.AddLayer(const L: string);
begin
  self.FListeLayers.Add(L);
end;
