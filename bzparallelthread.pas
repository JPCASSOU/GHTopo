...
(*================================================================================@br
 ...
 --------------------------------------------------------------------------------@br
  ...
  @bold(Description :)@br
   L'unité BZParallelThread contient un objet TBZParallelThread qui permet
   d'exécuter des opérations en parallele facilement grâce au multithreading.
...
Type
  ...
  { TBZParallelProc : Type de procedure simple d'execution du tratement à effectuer dans les threads }
  TBZParallelProc = procedure(Index: Integer; Data : Pointer);
  { TBZParallelProc : Type de procedure objet d'execution du tratement à effectuer dans les threads }
  TBZParallelProcEvent = procedure(Sender: TObject; Index: Integer ; Data : Pointer) of object;
  { TBZParallelProc : Type de procedure objet pour la progression de l'execution }
  TBZParallelProgressEvent = procedure(Sender: TObject; Index: Integer) of object;

  { TBZParallelProgressMode : Direction de traitement des données.
  ...
  { TBZParallelThread : Objet servant à l'initialisation de code paralleliser dans des threads en mode objet }
  ...
  public
    ...
    { Lancement du traitement à affectuer à partir de l'index "FromIndex" jusqu'à "ToIndex".
...
{ ParallelFor : Procedure de lancement d'un traitement paralléliser simple.
...
{ ParallelFor : Procedure de lancement d'un traitement paralléliser objet }
procedure ParallelFor(FromIndex, ToIndex : Int64; ParallelProc : TBZParallelProcEvent; const ThreadCount : Byte = 4); overload;
{ ParallelFor : Procedure de lancement d'un traitement paralléliser objet avec data }
...
{ ParallelFor : Procedure de lancement d'un traitement paralléliser objet avec data et évènement pour controler la progression de la tâche }
...
threadvar
  { BZCurrentParallelThread :  Variable globale servant à contenir le thread en cours d'execution. Utile dans le cas de synchronisation.
    Vous pouvez le définir dans vos propres descendants TThread }
