unit UnitWrapperDeclimagMacOSX;
// Wrapper pour le calcul des déclinaisons magnétiques
// via les algos de IGRF
// et la librairie de calcul en langage C.
// ------------------------------------------
// Version pour MacOSX (non fonctionnelle)
{$ERROR Version pour MacOSX (non fonctionnelle) }
...
{ TCalculDeclinaisonMagnetique }
 ...
 TCalculDeclinaisonMagnetique = class
  ...
  public
    ...
    function CalculerDeclimag(const Lon, Lat, Alt: double; const JJ, MM, AAAA: word): double; overload;
...
function TCalculDeclinaisonMagnetique.CalculerDeclimag(const Lon, Lat, Alt: double; const JJ, MM, AAAA: word): double; overload;
begin
...
begin
  ...
  if (FileExists(FICHIER_COF)) then
  begin
    Result := True;
    Exit;
  end;
  ShowMessage(Format('%s introuvable - Tentative de reconstruction du fichier',[FICHIER_COF]));
