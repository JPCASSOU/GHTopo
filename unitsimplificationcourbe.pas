...
//******************************************************************************
{ TSimplificationPolyline }

function TSimplificationPolyline.Initialiser(): boolean;
begin
  self.ClearListe();
end;

procedure TSimplificationPolyline.AddVertex(const QX, QY: double; const QTag: integer);
var
  E: TPoint2DfTagged;
begin
  E.X := QX;
...
function TSimplificationPolyline.GetVertex(const Idx: integer): TPoint2DfTagged;
begin
  Result := GetElement(Idx);
end;

function TSimplificationPolyline.PutVertex(const Idx: integer; const V: TPoint2DfTagged);
begin
  PutElement(Idx, V);
end;

procedure TSimplificationPolyline.Finaliser();
begin
  self.ClearListe();
end;
